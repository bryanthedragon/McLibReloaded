...
public class ResourceSelectorArgument<T> implements ArgumentType<Collection<Holder.Reference<T>>> {
    ...
    public static <T> Collection<Holder.Reference<T>> getSelectedResources(
        CommandContext<CommandSourceStack> p_394081_, String p_393093_, ResourceKey<? extends Registry<T>> p_397200_
    ) {
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> p_396736_, SuggestionsBuilder p_395731_) {
        return p_396736_.getSource() instanceof SharedSuggestionProvider sharedsuggestionprovider
            ? sharedsuggestionprovider.suggestRegistryElements(this.registryKey, SharedSuggestionProvider.ElementSuggestionType.ELEMENTS, p_395731_, p_396736_)
            : SharedSuggestionProvider.suggest(this.registryLookup.listElementIds().map(ResourceKey::location).map(ResourceLocation::toString), p_395731_);
    }
