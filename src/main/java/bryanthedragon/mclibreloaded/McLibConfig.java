...
public class McLibConfig 
{
    ...
    static 
    {
        ...
        ENABLE_BORDERS = BUILDER.comment("Enable GUI borders").define("enableBorders", false);
        TRACKPAD_DECIMAL_PLACES = BUILDER.comment("Trackpad decimal precision").defineInRange("trackpadDecimalPlaces", 6, 3, 31);
        BACKGROUND_IMAGE = BUILDER.comment("Background image path").define("backgroundImage", "");
        BUILDER.pop();
        SPEC = BUILDER.build();
    }
}
